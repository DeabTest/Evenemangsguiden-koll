#!/usr/bin/env python3
"""
Scrapar alla evenemang via din proxy och skriver ut de första raderna
av råsvaret direkt i loggen för att debugga JSON-parsningen.

• Anropar /rest-api/Evenemang/events via proxy
• Skriver ut de första 10 raderna av r.text
• Avkapslar Netlify-body om det finns
...
def fetch_page(page: int):
    ...
    # --- DEBUG: skriv ut råtextens första 10 rader direkt i loggen ---
    print("---- RAW PROXY RESPONSE START ----")
    for line in r.text.splitlines()[:10]:
        print(line)
    print("---- RAW PROXY RESPONSE END ----")
    ...
    try:
        ...
        raise RuntimeError(f"Ogiltig JSON på page={page}: {e}")
